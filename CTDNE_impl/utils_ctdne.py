...
def ctdne_embedding(graph, name):
    ...
    temporal_walks = temporal_rw.run(num_cw=num_cw,
                                     cw_size=context_window_size,
                                     max_walk_length=walk_length,
                                     walk_bias="exponential")
    
    print(f"Number of temporal random walks for '{name}': {len(temporal_walks)}")
...
# returns the classifier preprocessed so the data have 0 mean and unit variance
